...
/* Traced from the couple's own source art (`tmp/aaa/icons/sprig 1.svg`, potrace output). potrace
   wraps its output in `translate(0,600) scale(0.1,-0.1)`; that transform is baked into the path
   here and the viewBox re-origined on the drawing's own ink, so `IconBase`'s diagonal sizing reads
   a box that describes the artwork rather than potrace's working space.
...
interface SprigIconProps {
  /** Nominal optical span in px, measured on the diagonal. Rendered height is size x 0.6976. */
  size?: number;
  className?: string;
}

export function SprigIcon({ size, className }: SprigIconProps) {
  return (
